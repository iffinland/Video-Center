...
export const buildReactionIdentifier = (targetId, actorName) => {
...
export const buildReactionTargetPrefix = (targetId) => {
    const targetPart = targetId.replace(/^qvc-/, '').slice(0, 40);
    return `${REACTION_IDENTIFIER_PREFIX}${targetPart}-`;
};
// ── Reaction envelope builder ──────────────────────────────
export const buildReactionEnvelope = (targetId, targetType, actorName, walletAddress, state = 'active') => {
...
export const reduceReactionRecords = (records, identity) => {
    const diagnostics = [];
    const actors = {};
    let targetId = '';
    // Sort deterministically by created timestamp
    const sorted = [...records].sort((a, b) => a.metadata.created - b.metadata.created ||
...
// ── Display helpers ────────────────────────────────────────
export const resolveReactionDisplay = (state, currentUserName) => ({
    count: state.count,
    hasReacted: currentUserName ? normalizeName(currentUserName) in state.actors : false,
});
// ── Publication helpers ────────────────────────────────────
export const publishReactionEnvelope = async (envelope, publisher) => {
