...
import { buildV2Envelope, buildV2OwnerEditEnvelope, reduceV2RuntimeRecords, } from '../src/services/architectureV2/runtime.js';
...
assert.equal(videoState.authoritative.entities['qvc-v2-video-abc12345']?.title, 'Test Video');
...
const forgedVideo = { ...videoCreate, publisherName: 'mallory' };
...
assert.ok(forgedState.authoritative.entities['qvc-v2-video-abc12345'] === undefined, 'forged publisher rejected');
...
assert.equal(editedState.authoritative.entities[videoCreate.entityId]?.title, 'Updated Title');
...
assert.equal(badEditState.authoritative.entities[videoCreate.entityId]?.title, 'Test Video');
...
const dupCreate = { ...videoCreate, description: 'Different description' };
...
assert.ok(dupState.diagnostics.some((d) => d.code === 'UNAUTHORIZED_PUBLISHER'), 'duplicate with different publisher quarantined');
